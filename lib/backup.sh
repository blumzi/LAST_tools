...
export backup_default_target="euclid@euclid.weizmann.ac.il:/bigdata3/projects/last/data"
export backup_user=
export backup_host=
...
#
backup_check_all() {
    for stat_file in $(find /$(hostname -s)/data[12] -path "*/archive/*/proc/*/.status" 2>/dev/null | sort); do
