...
function postgres_check() {
    ...
    if ${this_is_last0}; then
        ...
        if type psql >/dev/null 2>&1; then
            message_success "The \"psql\" command is available"
        else
            message_failure "Missing the \"psql\" command."
            (( ret++ ))
    ...
    conf=~ocs/.pgpass
    if [ -r ${conf} ]; then
        local mode=$(stat -c '%a' ${conf})

        if [ "${mode}" != 600 ]; then
            message_failure "Access mode for ${conf} is ${mode} instead of 600"
            (( ret++ ))
        else
            message_success "Access mode for ${conf} is ${mode}"
        fi

        local expected_line="last0:5432:last_operational:postgres:postgres"
        read current_line < ${conf}
        if [ "${current_line}" != "${expected_line}" ]; then
            message_failure "Password file ${conf} is bad"
        else
            message_success "Password file ${conf} is OK"
        fi
    else
        message_failure "Missing ${conf}"
    fi
...
function postgres_enforce() {
    ...
    if ${this_is_last0}; then
        ...
        if  [ "$(dpkg --list | grep -Ec '(postgresql-client-14|postgresql-client-common|postgresql-common)')" -ge 3 ]; then
            message_success "The postgresql common and client packages are installed"
        else
            apt update
            apt install -y postgresql-client-14 postgresql-client-common postgresql-common
            message_success "Installed the postgresql common and client packages"
        fi
    fi
    ...
    if [ ! -r "${conf}" ]; then
        echo "last0:5432:last_operational:postgres:postgres" > "${conf}"
        chmod 600 "${conf}"
        chown ocs.ocs "${conf}"
    fi
}
